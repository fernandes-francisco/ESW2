...
namespace ESW2.Controllers
{
    ...
    public class AtivoFinanceiroController : Controller
    {
        ...
        [HttpGet]
        public async Task<IActionResult> Index(string nome, string tipo, decimal? montanteAplicado, bool? somenteAtivosAtuais, int[] bancos)
        {
            ...
            // Aplicar filtro de ativos na data atual
            ...
            {
                ativosQuery = ativosQuery.Where(a => a.id_deposito.HasValue && bancos.Contains(a.id_depositoNavigation.id_banco));
            }

            // Carregar lista de bancos para o dropdown
            ViewBag.Bancos = await _context.bancos
                .OrderBy(b => b.nome_banco)
                .ToListAsync();
            ...
            ViewBag.SelectedBancos = bancos ?? Array.Empty<int>();
            ...
            // Aplicar ordenação
            List<ativo_financeiro> ativosResult;
            if (somenteAtivosAtuais == true)
            {
                ativosResult = await ativosQuery
                    .Select(a => new
                    {
                        Ativo = a,
